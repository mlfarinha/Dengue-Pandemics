...
cond_tree_precip <- ctree(class ~. , data = train_precip)
cond_tree_precip
plot(cond_tree_precip)
plot(cond_tree_precip, type = "simple")

train.control <- trainControl(method = "repeatedcv", number = 5, repeats = 3)
set.seed(2020)
pred.cond_tree_precip <- train(train_precip[,c(1:12)],train_precip$class, method = "ctree",
                              trControl  = train.control,  metric = "Accuracy")
print(pred.cond_tree_precip) # 0.4217425


### Decision Tree for REANALYSIS_PRECIP_AMT_KG_PER_M2
cond_tree_reanalysis <- ctree(class ~. , data = train_reanalysis)
cond_tree_reanalysis
plot(cond_tree_reanalysis)
plot(cond_tree_reanalysis, type = "simple")

train.control <- trainControl(method = "repeatedcv", number = 5, repeats = 3)
set.seed(2020)
pred.cond_tree_reanalysis <- train(train_reanalysis[,c(1:12)],train_reanalysis$class, method = "ctree",
                                  trControl  = train.control,  metric = "Accuracy")
print(pred.cond_tree_reanalysis) # 0.4217425


### Decision Tree for STATION_PRECIP_MM
cond_tree_station <- ctree(class ~. , data = train_station)
cond_tree_station
plot(cond_tree_station)
plot(cond_tree_station, type = "simple")

train.control <- trainControl(method = "repeatedcv", number = 5, repeats = 3)
set.seed(2020)
pred.cond_tree_station <- train(train_station[,c(1:12)],train_station$class, method = "ctree",
                               trControl  = train.control,  metric = "Accuracy")
print(pred.cond_tree_station) # 0.4525563
